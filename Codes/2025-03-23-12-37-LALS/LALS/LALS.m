...
% Version: 1.0
%
% The LALS function applies a  Local Asymmetric Least Squares (LALS) baseline correction
% algorithm to a 1D data vector, allowing for different asymmetry (p) and smoothing
% parameters (lambda_asym) on specified intervals. Outside these intervals, a uniform
% smoothing penalty (lambdaWhit) is applied together with a global first-derivative penalty (mu).
% The method uses Iteratively Reweighted Least Squares (IRLS) to update weights based on the
% residuals and thereby adapt the baseline estimation locally.
%
